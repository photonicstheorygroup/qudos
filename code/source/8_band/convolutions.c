...
void convolutions( int N_shape, struct wave_vectors basis, struct supercell_geometry *supercell )
{
	...
	// Local strain and shape characteristic function convolutions
	struct convolution_char_strain convolutions_char_strain_local;
	// Central (reduced) portion of local strain and shape characteristic function convolutions
	...
	if ( myid < process_limit ) // Processes with IDs less than the process limit
	{
		...
		// Will hold local part of strain tensor components
		...
		// Extracting local parts of zero-padded strain tensor components from global
			...
			for ( int idx_y = 0; idx_y < sizes[1]; idx_y++ )
			{
				...
				for ( int idx_z = 0; idx_z < sizes[2]; idx_z++ )
				{
					...
					if ( global_row >= 0 && global_row < basis.N_Gx_T && idx_y >= 0 && idx_y <  basis.N_Gy_T && idx_z >= 0 && idx_z < basis.N_Gz_T )
					{
						...
						strain_G_local.xx[( global_row - local_start )*sizes[1]*sizes[2] + idx_y*sizes[2] + idx_z] = strain_G.xx[ ( global_row )*( basis.N_Gy_T )*( basis.N_Gz_T ) + ( idx_y )*( basis.N_Gz_T ) + ( idx_z )];
						strain_G_local.yy[( global_row - local_start )*sizes[1]*sizes[2] + idx_y*sizes[2] + idx_z] = strain_G.yy[ ( global_row )*( basis.N_Gy_T )*( basis.N_Gz_T ) + ( idx_y )*( basis.N_Gz_T ) + ( idx_z )];
						strain_G_local.zz[( global_row - local_start )*sizes[1]*sizes[2] + idx_y*sizes[2] + idx_z] = strain_G.zz[ ( global_row )*( basis.N_Gy_T )*( basis.N_Gz_T ) + ( idx_y )*( basis.N_Gz_T ) + ( idx_z )];
						strain_G_local.yz[( global_row - local_start )*sizes[1]*sizes[2] + idx_y*sizes[2] + idx_z] = strain_G.yz[ ( global_row )*( basis.N_Gy_T )*( basis.N_Gz_T ) + ( idx_y )*( basis.N_Gz_T ) + ( idx_z )];
						strain_G_local.xz[( global_row - local_start )*sizes[1]*sizes[2] + idx_y*sizes[2] + idx_z] = strain_G.xz[ ( global_row )*( basis.N_Gy_T )*( basis.N_Gz_T ) + ( idx_y )*( basis.N_Gz_T ) + ( idx_z )];
						strain_G_local.xy[( global_row - local_start )*sizes[1]*sizes[2] + idx_y*sizes[2] + idx_z] = strain_G.xy[ ( global_row )*( basis.N_Gy_T )*( basis.N_Gz_T ) + ( idx_y )*( basis.N_Gz_T ) + ( idx_z )];
		...
		// Local arrays to store element by element multiplcation of FFT arrays
		...
		// Local arrays to then store local extracted central portion
		...
		convolutions_char_strain_reduced_local.char_hydrostatic = ( complex double* ) calloc( local_size_reduced, sizeof( complex double ) );
		convolutions_char_strain_reduced_local.char_biaxial = ( complex double* ) calloc( local_size_reduced, sizeof( complex double ) );
		...
		for( int idx_shape = 1; idx_shape < N_shape; idx_shape++ )
		{
				...
				for ( int idx_y = 0; idx_y < sizes[1]; idx_y++ )
				{
					...
					for ( int idx_z = 0; idx_z < sizes[2]; idx_z++ )
					{
						...
						if ( global_row >= 0 && global_row < basis.N_Gx_T && idx_y >= 0 && idx_y <  basis.N_Gy_T && idx_z >= 0 && idx_z < basis.N_Gz_T )
						{
							...
							characteristic_function_G_local.chi[( global_row - local_start )*sizes[1]*sizes[2] + idx_y*sizes[2] + idx_z] = characteristic_function_G[idx_shape].chi[ ( global_row )*( basis.N_Gy_T )*( basis.N_Gz_T ) + ( idx_y )*( basis.N_Gz_T ) + ( idx_z ) ];

						}
						else
						{

							characteristic_function_G_local.chi[( global_row - local_start )*sizes[1]*sizes[2] + idx_y*sizes[2] + idx_z] = 0;
			...
			for ( int idx_x = 0; idx_x < local_rows_reduced; idx_x++ )
			{
				...
				for ( int idx_y = 0; idx_y < basis.N_Gy_T; idx_y++ )
				{
					...
					for ( int idx_z = 0; idx_z < basis.N_Gz_T; idx_z++ )
					{
						...
						convolutions_char_strain_reduced_local.char_xx[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z] = convolutions_char_strain_local.char_xx[ ( idx_x + local_start_reduced )*( sizes[1] )*( sizes[2] ) + ( idx_y + basis.N_Gy_T / 2 )*( sizes[2] ) + ( idx_z + basis.N_Gz_T / 2 ) ];
						convolutions_char_strain_reduced_local.char_yy[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z] = convolutions_char_strain_local.char_yy[ ( idx_x + local_start_reduced )*( sizes[1] )*( sizes[2] ) + ( idx_y + basis.N_Gy_T / 2 )*( sizes[2] ) + ( idx_z + basis.N_Gz_T / 2 ) ];
						convolutions_char_strain_reduced_local.char_zz[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z] = convolutions_char_strain_local.char_zz[ ( idx_x + local_start_reduced )*( sizes[1] )*( sizes[2] ) + ( idx_y + basis.N_Gy_T / 2 )*( sizes[2] ) + ( idx_z + basis.N_Gz_T / 2 ) ];
						convolutions_char_strain_reduced_local.char_yz[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z] = convolutions_char_strain_local.char_yz[ ( idx_x + local_start_reduced )*( sizes[1] )*( sizes[2] ) + ( idx_y + basis.N_Gy_T / 2 )*( sizes[2] ) + ( idx_z + basis.N_Gz_T / 2 ) ];
						convolutions_char_strain_reduced_local.char_xz[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z] = convolutions_char_strain_local.char_xz[ ( idx_x + local_start_reduced )*( sizes[1] )*( sizes[2] ) + ( idx_y + basis.N_Gy_T / 2 )*( sizes[2] ) + ( idx_z + basis.N_Gz_T / 2 ) ];
						convolutions_char_strain_reduced_local.char_xy[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z] = convolutions_char_strain_local.char_xy[ ( idx_x + local_start_reduced )*( sizes[1] )*( sizes[2] ) + ( idx_y + basis.N_Gy_T / 2 )*( sizes[2] ) + ( idx_z + basis.N_Gz_T / 2 ) ];
						convolutions_char_strain_reduced_local.char_hydrostatic[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z] = convolutions_char_strain_reduced_local.char_xx[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z] + convolutions_char_strain_reduced_local.char_yy[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z] + convolutions_char_strain_reduced_local.char_zz[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z];
						convolutions_char_strain_reduced_local.char_biaxial[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z] = convolutions_char_strain_reduced_local.char_xx[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z] + convolutions_char_strain_reduced_local.char_yy[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z] - 2.0*convolutions_char_strain_reduced_local.char_zz[idx_x*basis.N_Gy_T*basis.N_Gz_T + idx_y*basis.N_Gz_T + idx_z];
			...
			MPI_Allgatherv( convolutions_char_strain_reduced_local.char_xx, local_size_reduced, MPI_DOUBLE_COMPLEX, convolutions_char_strain[idx_shape].char_xx, recvcounts_convolutions, displs_convolutions, MPI_DOUBLE_COMPLEX, MPI_COMM_WORLD );
			MPI_Allgatherv( convolutions_char_strain_reduced_local.char_yy, local_size_reduced, MPI_DOUBLE_COMPLEX, convolutions_char_strain[idx_shape].char_yy, recvcounts_convolutions, displs_convolutions, MPI_DOUBLE_COMPLEX, MPI_COMM_WORLD );
			MPI_Allgatherv( convolutions_char_strain_reduced_local.char_zz, local_size_reduced, MPI_DOUBLE_COMPLEX, convolutions_char_strain[idx_shape].char_zz, recvcounts_convolutions, displs_convolutions, MPI_DOUBLE_COMPLEX, MPI_COMM_WORLD );
			MPI_Allgatherv( convolutions_char_strain_reduced_local.char_yz, local_size_reduced, MPI_DOUBLE_COMPLEX, convolutions_char_strain[idx_shape].char_yz, recvcounts_convolutions, displs_convolutions, MPI_DOUBLE_COMPLEX, MPI_COMM_WORLD );
			MPI_Allgatherv( convolutions_char_strain_reduced_local.char_xz, local_size_reduced, MPI_DOUBLE_COMPLEX, convolutions_char_strain[idx_shape].char_xz, recvcounts_convolutions, displs_convolutions, MPI_DOUBLE_COMPLEX, MPI_COMM_WORLD );
			MPI_Allgatherv( convolutions_char_strain_reduced_local.char_xy, local_size_reduced, MPI_DOUBLE_COMPLEX, convolutions_char_strain[idx_shape].char_xy, recvcounts_convolutions, displs_convolutions, MPI_DOUBLE_COMPLEX, MPI_COMM_WORLD );
			MPI_Allgatherv( convolutions_char_strain_reduced_local.char_hydrostatic, local_size_reduced, MPI_DOUBLE_COMPLEX, convolutions_char_strain[idx_shape].char_hydrostatic, recvcounts_convolutions, displs_convolutions, MPI_DOUBLE_COMPLEX, MPI_COMM_WORLD );
			MPI_Allgatherv( convolutions_char_strain_reduced_local.char_biaxial, local_size_reduced, MPI_DOUBLE_COMPLEX, convolutions_char_strain[idx_shape].char_biaxial, recvcounts_convolutions, displs_convolutions, MPI_DOUBLE_COMPLEX, MPI_COMM_WORLD );
		...
		free( convolutions_char_strain_reduced_local.char_hydrostatic );
		free( convolutions_char_strain_reduced_local.char_biaxial );
	...
	else // Processes with IDs greater than or equal to the process limit
	{
		...
		// Gathering local reduced convolution array elements for each shape
		for( int idx_shape = 1; idx_shape < N_shape; idx_shape++ )
		{
