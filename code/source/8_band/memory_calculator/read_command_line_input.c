// QUantum Dot Open-source Simulator (QUDOS)
//
// Copyright (c) 2025
// Tommy Murphy and Christopher A. Broderick
//
// This file is part of QUDOS.
//
// QUDOS is free software: you can redistribute it and/or modify it under the terms of the
// GNU General Public License as published by the Free Software Foundation, either
// version 3 of the License, or (at your discretion) any later version.
//
// QUDOS is distributed in the hope that it will be useful, but without any warranty and
// without an implied warranty of merchantability or fitness for a particular purpose.
// See the GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License along with QUDOS.
// If not, see <https://www.gnu.org/licenses/>.
//
// Alternative commercial licensing terms may be available from the copyright holders upon request.

#include "8_band.h"

void read_command_line_input( int argc, char *argv[], int *N_shape, int *N_Gx, int *N_Gy, int *N_Gz, int *strain_region_type, int *N_S_Gx, int *N_S_Gy, int *N_S_Gz, int *N_vb, int *N_cb, double *T, double *Fx, double *Fy, double *Fz, double *Kx, double *Ky, double *Kz, int *N_grid, int *include_strain, int *isotropic, int *include_shape_elastic_constants, int *shape_elastic_constants_order, int *include_piezo, int *optimise, int *include_overlaps, int *include_localisation, int *include_MME, int *include_CME, int *coulomb_region_type, int *N_C_Gx, int *N_C_Gy, int *N_C_Gz, int *softening, double *sigma_softening, int *directions, int* cross_sections, int *N_grid_cross_sections, int *directions_cross_sections, int *enable_3D, int *N_grid_3D, int *input_check )
{

	//////////////////////////////////
	//// Read command line inputs ////
	//////////////////////////////////

	// WARNING! Command line inputs override those set from parameters.in
	// Arguments must be exactly of the form '-parameter=value', where the parameters have the same names as those read from parameters.in
	// NOTE: Total number of input parameters is given by N_PARAM in 8_band.h

	for ( int param = 1; param < argc; param++ )
	{

		if (argv[param][0] != '-') 
		{
            
            printf( "Error. Invalid argument format: %s\nArguments must begin with '-'.\n", argv[param] );
        	MPI_Abort( MPI_COMM_WORLD, 1 );

        }

		char *arg = argv[param] + 1;  // Skip leading '-'
        char *equals = strchr( arg, '=' ); // Check for '='
        if ( !equals )
		{

            printf( "Error. Invalid argument (missing '='): %s\n", argv[param]);
        	MPI_Abort( MPI_COMM_WORLD, 1 );

		}
        
		*equals = '\0';  // Split into key/value
        char *key = arg;
        char *value = equals + 1;

		// Identify and set command line arguments
		if ( strcmp( key, "N_shape" ) == 0 ) // Number of shapes in supercell (including supercell)
		{

			*N_shape = ( int ) strtol( value, &endptr, 10 );
			input_check[0] = 1;
		
		}
		else if ( strcmp( key, "M_Gx" ) == 0 ) // Number of plane waves for electronic structure supercell
		{

			*N_Gx = 2*( ( int ) strtol( value, &endptr, 10 ) ) + 1; // N_Gx = 2*M_Gx + 1 = Total no. of plane waves along x
			input_check[1] = 1;
		
		}
		else if ( strcmp( key, "M_Gy" ) == 0 ) // Number of plane waves for electronic structure supercell
		{

			*N_Gy = 2*( ( int ) strtol( value, &endptr, 10 ) ) + 1; // N_Gy = 2*M_Gy + 1 = Total no. of plane waves along y
			input_check[2] = 1;
		
		}
		else if ( strcmp( key, "M_Gz" ) == 0 ) // Number of plane waves for electronic structure supercell
		{

			*N_Gz = 2*( ( int ) strtol( value, &endptr, 10 ) ) + 1; // N_Gz = 2*M_Gz + 1 = Total no. of plane waves along z
			input_check[3] = 1;
		
		}
		else if ( strcmp( key, "Strain_supercell_type" ) == 0 ) // Indicates whether to use free strain supercell lengths (0) or to set them to be integer multiples of the electronic structure supercell lengths (1) allowing much faster strain and piezoelectric potential conversion from the strain supercell to the electronic structure supercell.
		{

			*strain_region_type = ( int ) strtol( value, &endptr, 10 );
			input_check[4] = 1;
		
		}
		else if ( strcmp( key, "M_S_Gx" ) == 0 ) // Plane wave basis set size for the expanded strain supercell.
		{

			*N_S_Gx = 2*( ( int ) strtol( value, &endptr, 10 ) ) + 1; // N_S_Gx = 2*M_S_Gx + 1 = Total no. of plane waves along x
			input_check[5] = 1;
		
		}
		else if ( strcmp( key, "M_S_Gy" ) == 0 ) // Plane wave basis set size for the expanded strain supercell.
		{

			*N_S_Gy = 2*( ( int ) strtol( value, &endptr, 10 ) ) + 1; // N_S_Gy = 2*M_S_Gy + 1 = Total no. of plane waves along y
			input_check[6] = 1;
		
		}
		else if ( strcmp( key, "M_S_Gz" ) == 0 ) // Plane wave basis set size for the expanded strain supercell.
		{

			*N_S_Gz = 2*( ( int ) strtol( value, &endptr, 10 ) ) + 1; // N_S_Gz = 2*M_S_Gz + 1 = Total no. of plane waves along z
			input_check[7] = 1;
		
		}
		else if ( strcmp( key, "N_VB" ) == 0 ) // Number of valence states to compute
		{

			*N_vb = ( int ) strtol( value, &endptr, 10 ); // Excluding factor of 2 to account for spin degeneracy
			input_check[8] = 1;
		
		}
		else if ( strcmp( key, "N_CB" ) == 0 ) // Number of conduction states to compute
		{

			*N_cb = ( int ) strtol( value, &endptr, 10 ); // Excluding factor of 2 to account for spin degeneracy
			input_check[9] = 1;
		
		}
		else if ( strcmp( key, "T" ) == 0 ) // System temperature [K]
		{

			*T = strtod( value, &endptr );
			input_check[10] = 1;
		
		}
		else if ( strcmp( key, "Fx" ) == 0 ) // Applied electric field x component - read in kV/cm and convert to V/Angstrom
		{

			*Fx = 1.0e-05*strtod( value, &endptr ); // Read in kV/cm and convert to V/Angstrom
			input_check[11] = 1;
		
		}
		else if ( strcmp( key, "Fy" ) == 0 ) // Applied electric field y component - read in kV/cm and convert to V/Angstrom
		{

			*Fy = 1.0e-05*strtod( value, &endptr ); // Read in kV/cm and convert to V/Angstrom
			input_check[12] = 1;
		
		}
		else if ( strcmp( key, "Fz" ) == 0 ) // Applied electric field z component - read in kV/cm and convert to V/Angstrom
		{

			*Fz = 1.0e-05*strtod( value, &endptr ); // Read in kV/cm and convert to V/Angstrom
			input_check[13] = 1;
		
		}
		else if ( strcmp( key, "Kx" ) == 0 ) // Supercell wave vector x component (for miniband dispersion) - read in nm^-1 and convert to Angstrom^-1
		{

			*Kx = 0.1*strtod( value, &endptr ); 
			input_check[14] = 1;
		
		}
		else if ( strcmp( key, "Ky" ) == 0 ) // Supercell wave vector y component (for miniband dispersion) - read in nm^-1 and convert to Angstrom^-1
		{

			*Ky = 0.1*strtod( value, &endptr ); 
			input_check[15] = 1;
		
		}
		else if ( strcmp( key, "Kz" ) == 0 ) // Supercell wave vector z component (for miniband dispersion) - read in nm^-1 and convert to Angstrom^-1
		{

			*Kz = 0.1*strtod( value, &endptr );
			input_check[16] = 1;
		
		}
		else if ( strcmp( key, "N_line" ) == 0 ) // Number of real space grid points for linescans
		{

			*N_grid = ( int ) strtol( value, &endptr, 10 );
			input_check[17] = 1;
		
		}
		else if ( strcmp( key, "Strain" ) == 0 ) // Include strain in calculation
		{

			*include_strain = ( int ) strtol( value, &endptr, 10 ); // = 0 for no strain, = 1 to include strain
			input_check[18] = 1;
		
		}
		else if ( strcmp( key, "Isotropic" ) == 0 ) // Isotropic approximation for elastic tensor
		{

			*isotropic = ( int ) strtol( value, &endptr, 10 ); // = 0 for full anisotropic elastic tensor, = 1 for isotropic approximation to elastic tensor
			input_check[19] = 1;
		
		}
		else if ( strcmp( key, "Elastic_const" ) == 0 ) //  Material dependent elastic constants
		{

			*include_shape_elastic_constants = ( int ) strtol( value, &endptr, 10 ); // = 0 for matrix material elastic constants, = 1 for shape dependent elastic constants
			input_check[20] = 1;
		
		}
		else if ( strcmp( key, "Elastic_const_order" ) == 0 ) // Correction order for position-dependent elastic constants
		{

			*shape_elastic_constants_order = ( int ) strtol( value, &endptr, 10 ); // order of correction to strain tensor for shape elastic constants
			input_check[21] = 1;
		
		}
		else if ( strcmp( key, "Piezo" ) == 0 ) // Include piezoelectric potential in calculation
		{

			*include_piezo = ( int ) strtol( value, &endptr, 10 ); // = 0 for no piezoelectricity, = 1 for 1st order only, = 2 for 1st and 2nd order, = 3 for 2nd order only
			input_check[22] = 1;
		
		}
		else if ( strcmp( key, "Overlaps" ) == 0 ) // Include state overlaps calculation
		{

			*include_overlaps = ( int ) strtol( value, &endptr, 10 ); // = 0 for no overlaps, = 1 for overlaps
			input_check[23] = 1;
		
		}
		else if ( strcmp( key, "Localisation" ) == 0 ) // Include shape carrier localisation calculation
		{

			*include_localisation = ( int ) strtol( value, &endptr, 10 ); // = 0 for no shape carrier localisation, = 1 for shape carrier localisation
			input_check[24] = 1;
		
		}
		else if ( strcmp( key, "MME" ) == 0 ) // Include interband momentum matrix element calculation
		{

			*include_MME = ( int ) strtol( value, &endptr, 10 ); // = 0 for no momentum matrix elements, = 1 for momentum matrix elements
			input_check[25] = 1;
		
		}
		else if ( strcmp( key, "CME" ) == 0 ) // Include coulomb matrix elements in calculation with optional corrections
		{

			*include_CME = ( int ) strtol( value, &endptr, 10 ); // = 0 for no coulomb matrix elements, = 1 for coulomb matrix elements, = 2 for coulomb matrix elements with monopole corrections, = 3 for coulomb matrix elements with dipole corrections, = 4 for coulomb matrix elements with quadrupole corrections
			input_check[26] = 1;
		
		}
		else if ( strcmp( key, "Coulomb_supercell_type" ) == 0 ) // Indicates whether to use free Coulomb supercell lengths (0) or to set them to be integer multiples of the electronic structure supercell lengths (1) allowing much faster upsampling.
		{

			*coulomb_region_type = ( int ) strtol( value, &endptr, 10 );
			input_check[27] = 1;
		
		}
		else if ( strcmp( key, "M_C_Gx" ) == 0 ) // Plane wave basis set size for the expanded Coulomb supercell.
		{

			*N_C_Gx = 2*( ( int ) strtol( value, &endptr, 10 ) ) + 1; // N_C_Gx = 2*M_C_Gx + 1 = Total no. of plane waves along x
			input_check[28] = 1;
		
		}
		else if ( strcmp( key, "M_C_Gy" ) == 0 ) // Plane wave basis set size for the expanded Coulomb supercell.
		{

			*N_C_Gy = 2*( ( int ) strtol( value, &endptr, 10 ) ) + 1; // N_C_Gy = 2*M_C_Gy + 1 = Total no. of plane waves along y
			input_check[29] = 1;
		
		}
		else if ( strcmp( key, "M_C_Gz" ) == 0 ) // Plane wave basis set size for the expanded Coulomb supercell.
		{

			*N_C_Gz = 2*( ( int ) strtol( value, &endptr, 10 ) ) + 1; // N_C_Gz = 2*M_C_Gz + 1 = Total no. of plane waves along z
			input_check[30] = 1;
		
		}
		else if ( strcmp( key, "Softening" ) == 0 ) // Interface softening via Gaussian convolution
		{

			*softening = ( int ) strtol( value, &endptr, 10 ); // = 1 for interface softening, = 0 for "hard" interface calculation
			input_check[31] = 1;
		
		}
		else if ( strcmp( key, "Sigma_softening" ) == 0 ) // Include state overlaps calculation
		{

			*sigma_softening = 10.0*strtod( value, &endptr     ); // Standard deviation of Gaussian used for interface softening, read in nm and convert to Angstrom (default: 1.0 Angstrom = 0.1 nm)
			input_check[32] = 1;
		
		}
		else if ( strcmp( key, "Linescan_100" ) == 0 ) // Set directions along which to compute real space linescans (linescans go through centre of supercell)
		{

			directions[0] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute linescan along [100]
			input_check[33] = 1;
		
		}
		else if ( strcmp( key, "Linescan_010" ) == 0 ) // Set directions along which to compute real space linescans (linescans go through centre of supercell)
		{

			directions[1] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute linescan along [010]
			input_check[34] = 1;
		
		}
		else if ( strcmp( key, "Linescan_001" ) == 0 ) // Set directions along which to compute real space linescans (linescans go through centre of supercell)
		{

			directions[2] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute linescan along [001]
			input_check[35] = 1;
		
		}
		else if ( strcmp( key, "Linescan_110" ) == 0 ) // Set directions along which to compute real space linescans (linescans go through centre of supercell)
		{

			directions[3] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute linescan along [110]
			input_check[36] = 1;
		
		}
		else if ( strcmp( key, "Linescan_101" ) == 0 ) // Set directions along which to compute real space linescans (linescans go through centre of supercell)
		{

			directions[4] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute linescan along [101]
			input_check[37] = 1;
		
		}
		else if ( strcmp( key, "Linescan_011" ) == 0 ) // Set directions along which to compute real space linescans (linescans go through centre of supercell)
		{

			directions[5] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute linescan along [011]
			input_check[38] = 1;
		
		}
		else if ( strcmp( key, "Linescan_111" ) == 0 ) // Set directions along which to compute real space linescans (linescans go through centre of supercell)
		{

			directions[6] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute linescan along [111]
			input_check[39] = 1;
		
		}
		else if ( strcmp( key, "Cross_sections" ) == 0 ) // Indicate whether to output cross-sections of envelope functions
		{

			*cross_sections = ( int ) strtol( value, &endptr, 10 ); // = 1 for cross sections, = 0 for no cross sections
			input_check[40] = 1;
		
		}
		else if ( strcmp( key, "N_cross" ) == 0 ) // Number of real-space grid points for each of the two dimensions for cross-sections
		{

			*N_grid_cross_sections = ( int ) strtol( value, &endptr, 10 );
			input_check[41] = 1;
		
		}
		else if ( strcmp( key, "Cross_section_100" ) == 0 ) // Set normals of which to compute real space cross-sections (cross-sections go through centre of supercell)
		{

			directions_cross_sections[0] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute cross-section with normal along [100]
			input_check[42] = 1;
		
		}
		else if ( strcmp( key, "Cross_section_010" ) == 0 ) // Set normals of which to compute real space cross-sections (cross-sections go through centre of supercell)
		{

			directions_cross_sections[1] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute cross-section with normal along [010]
			input_check[43] = 1;
		
		}
		else if ( strcmp( key, "Cross_section_001" ) == 0 ) // Set normals of which to compute real space cross-sections (cross-sections go through centre of supercell)
		{

			directions_cross_sections[2] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute cross-section with normal along [001]
			input_check[44] = 1;
		
		}
		else if ( strcmp( key, "Cross_section_m110" ) == 0 ) // Set normals of which to compute real space cross-sections (cross-sections go through centre of supercell)
		{

			directions_cross_sections[3] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute cross-section with normal along [m110]
			input_check[45] = 1;
		
		}
		else if ( strcmp( key, "Cross_section_110" ) == 0 ) // Set normals of which to compute real space cross-sections (cross-sections go through centre of supercell)
		{

			directions_cross_sections[4] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute cross-section with normal along [110]
			input_check[46] = 1;
		
		}
		else if ( strcmp( key, "Cross_section_m101" ) == 0 ) // Set normals of which to compute real space cross-sections (cross-sections go through centre of supercell)
		{

			directions_cross_sections[5] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute cross-section with normal along [m101]
			input_check[47] = 1;
		
		}
		else if ( strcmp( key, "Cross_section_0m11" ) == 0 ) // Set normals of which to compute real space cross-sections (cross-sections go through centre of supercell)
		{

			directions_cross_sections[6] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute cross-section with normal along [0m11]
			input_check[48] = 1;
		
		}
		else if ( strcmp( key, "Cross_section_111" ) == 0 ) // Set normals of which to compute real space cross-sections (cross-sections go through centre of supercell)
		{

			directions_cross_sections[7] = ( int ) strtol( value, &endptr, 10 ); // = 1 to compute cross-section with normal along [111]
			input_check[49] = 1;
		
		}
		else if ( strcmp( key, "3D_output" ) == 0 ) // Indicate whether to output 3D realspace plots
		{

			*enable_3D = ( int ) strtol( value, &endptr, 10 ); // = 1 to enable, = 0 to disable
			input_check[50] = 1;
		
		}
		else if ( strcmp( key, "N_3D" ) == 0 ) // Number of real space grid points in each direction for 3D outputs
		{

			*N_grid_3D = ( int ) strtol( value, &endptr, 10 );
			input_check[51] = 1;
		
		}
		else if ( strcmp( key, "Eigensolver" ) == 0 ) // Number of real space grid points in each direction for 3D outputs
		{

			*optimise = ( int ) strtol( value, &endptr, 10 );
			input_check[52] = 1;
		
		}

	}


	int char_size = 64; 			// Size of char holding parameter name
	char input_identifier[N_PARAM][char_size]; // Array to check that all input parameters have been provided
	
	strcpy( input_identifier[0], "N_shape" );
	strcpy( input_identifier[1], "M_Gx" );
	strcpy( input_identifier[2], "M_Gy" );
	strcpy( input_identifier[3], "M_Gz" );
	strcpy( input_identifier[4], "Strain_supercell_type" );
	strcpy( input_identifier[5], "M_S_Gx" );
	strcpy( input_identifier[6], "M_S_Gy" );
	strcpy( input_identifier[7], "M_S_Gz" );
	strcpy( input_identifier[8], "N_VB" );
	strcpy( input_identifier[9], "N_CB" );
	strcpy( input_identifier[10], "T" );
	strcpy( input_identifier[11], "Fx" );
	strcpy( input_identifier[12], "Fy" );
	strcpy( input_identifier[13], "Fz" );
	strcpy( input_identifier[14], "Kx" );
	strcpy( input_identifier[15], "Ky" );
	strcpy( input_identifier[16], "Kz" );
	strcpy( input_identifier[17], "N_line" );
	strcpy( input_identifier[18], "Strain" );
	strcpy( input_identifier[19], "Isotropic" );
	strcpy( input_identifier[20], "Elastic_const" );
	strcpy( input_identifier[21], "Elastic_const_order" );
	strcpy( input_identifier[22], "Piezo" );
	strcpy( input_identifier[23], "Overlaps" );
	strcpy( input_identifier[24], "Localisation" );
	strcpy( input_identifier[25], "MME" );
	strcpy( input_identifier[26], "CME" );
	strcpy( input_identifier[27], "Coulomb_supercell_type" );
	strcpy( input_identifier[28], "M_C_Gx" );
	strcpy( input_identifier[29], "M_C_Gy" );
	strcpy( input_identifier[30], "M_C_Gz" );
	strcpy( input_identifier[31], "Softening" );
	strcpy( input_identifier[32], "Sigma_softening" );
	strcpy( input_identifier[33], "Linescan_100" );
	strcpy( input_identifier[34], "Linescan_010" );
	strcpy( input_identifier[35], "Linescan_001" );
	strcpy( input_identifier[36], "Linescan_110" );
	strcpy( input_identifier[37], "Linescan_101" );
	strcpy( input_identifier[38], "Linescan_011" );
	strcpy( input_identifier[39], "Linescan_111" );
	strcpy( input_identifier[40], "Cross_sections" );
	strcpy( input_identifier[41], "N_cross" );
	strcpy( input_identifier[42], "Cross_section_100" );
	strcpy( input_identifier[43], "Cross_section_010" );
	strcpy( input_identifier[44], "Cross_section_001" );
	strcpy( input_identifier[45], "Cross_section_m110" );
	strcpy( input_identifier[46], "Cross_section_110" );
	strcpy( input_identifier[47], "Cross_section_m101" );
	strcpy( input_identifier[48], "Cross_section_0m11" );
	strcpy( input_identifier[49], "Cross_section_111" );
	strcpy( input_identifier[50], "3D_output" );
	strcpy( input_identifier[51], "N_3D" );
	strcpy( input_identifier[52], "Eigensolver" );

	// Abort if input parameter is not set by either parameters.in or the command line arguments
	int input_error = 0;
	for ( int param = 0; param < N_PARAM; param++ )
	{

		if ( input_check[param] == 0 )
		{

			printf( "Error. Argument '%s' not set.\n", input_identifier[param] );
			input_error = 1;

		}

	}
	if ( input_error == 1 )
	{

		MPI_Abort( MPI_COMM_WORLD, 1 );

	}

}
