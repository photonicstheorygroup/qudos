...
void strain_region_reciprocal_space( int N_shape, struct wave_vectors basis, struct supercell_geometry *supercell, struct material_params *parameters )
{
	...
	local_mem += ( 6 )*( local_size ) *sizeof( complex double ); // Adding memory to store local portions of strain tensor components for strain supercell
	

	if ( include_shape_elastic_constants == 0 ) // If using position-independent elastic constants
	{

		local_mem_peak = local_mem; // Peak memory usage

	}
	else	// If using position-dependent elastic constants
	{

		local_mem += ( 6 )*( basis.N_Gx_T*basis.N_Gy_T*basis.N_Gz_T ) *sizeof( complex double );	// Adding memory to store updated global strain tensor after each order correction


		local_mem += ( 6 )*( basis.N_Gx_T*basis.N_Gy_T*basis.N_Gz_T ) *sizeof( complex double );	// Adding memory to store global convolution of shape characteristic function with corrected strain tensor after each order correction, for calculation of next order correction

		local_mem += ( 6 )*( local_size ) *sizeof( complex double );	// Adding memory to store local strain correction

		if ( shape_elastic_constants_order > 0) // If correction order is 1 or more
		{

			// Variables for MKL cluster DFT library
			...
			if ( myid < process_limit )
			{
				...
				local_mem += ( 7 )*( local_memory_size) *sizeof( complex double ); // Adding memory for local parts of current order correction strain tensor and shape characteristic function
				...
				local_mem += ( 6 )*( local_memory_size) *sizeof( complex double );  // Adding memory for element by element multiplcation of FFT arrays


				local_mem += ( 6 )*( local_size_reduced) *sizeof( complex double );	// Adding memory to store local extracted central portions after convolutoion
				local_mem_peak = local_mem;											// Peak memory usage
				...
				local_mem -= ( 6 )*( local_size_reduced) *sizeof( complex double );
		...
		local_mem -= ( 6 )*( local_size ) *sizeof( complex double );
