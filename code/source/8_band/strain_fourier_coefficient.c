...
complex double strain_fourier_coefficient( int component, int idx_shape, int idx_basis, double Gx, double Gy, double Gz, struct supercell_geometry *supercell, struct material_params *parameters, int correction_order, struct convolution_char_strain *convolutions_char_strain_correction )
{
	...
	if ( correction_order == -1 ) // Only occurs for the initial contribution in the case of position-dependent elastic constants
	{
		...
		result = eps0*characteristic_function_strain_G[idx_shape].chi[idx_basis]*kronecker_delta( idx_row, idx_col );

	}
	else if ( correction_order == 0 ) // Occurs for cases of both position-independent and position-dependent elastic constants
	{

		result = eps0*characteristic_function_strain_G[idx_shape].chi[idx_basis];
