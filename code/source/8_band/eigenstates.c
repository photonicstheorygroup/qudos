...
void eigenstates( int N_shape, int *N_states, struct vector F, struct wave_vectors basis, struct supercell_geometry *supercell, struct material_params *parameters, struct eigenstate *eigenstate_vb, struct eigenstate *eigenstate_cb )
{
	...
	complex double *H; // local part of Hamiltonian matrix
	
	if (PACKED && nprocs == 1) // using 1 MPI process & packed storage
	{
	...
	if ( myid == 0 )
	{
		...
		printf( "\tCalculating Hamiltonian matrix elements...\n");

	}
	
	double start, finish, elapsed; // Variables to measure runtime of the different calculational steps	
	
	start = MPI_Wtime();

	// Calculating Hamiltonian matrix elements
	hamiltonian_8_band( N_shape, F, basis, supercell, parameters, H );
	
	finish = MPI_Wtime();
    elapsed = finish - start;
	component_run_times.ham_elements = elapsed;
	...
	if ( myid == 0 )
	{
		...
		printf( "\tDiagonalising Hamiltonian...\n");
	...
	finish = MPI_Wtime();
    elapsed = finish - start;
	component_run_times.ham_diag = elapsed;
	...
	// Collect computed eigenstates
	collect_eigenstates( N_states, basis, eigval, eigvec, eigenstate_vb, eigenstate_cb );
