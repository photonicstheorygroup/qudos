...
void write_envelope_functions( int *N_states, int N_grid, double *x, double *y, double *z, struct wave_vectors basis, struct eigenstate *eigenstate_vb, struct eigenstate *eigenstate_cb, int direction )
{
	...
	int idx_state, idx_grid, size = 80;
	char *str1, *str2;
	...
	for( idx_state = 2*N_states[0] - 1; idx_state > -1; idx_state-- ) // Write out valence bands in order of decreasing energy
	{
		...
		strcpy( str1, "envelope_function_vb_" );
		switch( direction )
		{
			...
			case 4: // Linescan along [101]
				...
				size = sprintf( str2, "%d_101.dat", 2*N_states[0] - idx_state );

				break;

			case 5: // Linescan along [011]

				size = sprintf( str2, "%d_011.dat", 2*N_states[0] - idx_state );
		...
		for( idx_grid = 0; idx_grid < N_grid; idx_grid++ )
		{
			...
			fprintf( fp1, "%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\n",
														0.1*x[idx_grid], 0.1*y[idx_grid], 0.1*z[idx_grid], eigenstate_vb[idx_state].E,
														sqrt( 1000.0 )*creal( eigenstate_vb[idx_state].F_4[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_vb[idx_state].F_4[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_vb[idx_state].F_8[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_vb[idx_state].F_8[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_vb[idx_state].F_3[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_vb[idx_state].F_3[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_vb[idx_state].F_7[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_vb[idx_state].F_7[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_vb[idx_state].F_2[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_vb[idx_state].F_2[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_vb[idx_state].F_6[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_vb[idx_state].F_6[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_vb[idx_state].F_1[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_vb[idx_state].F_1[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_vb[idx_state].F_5[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_vb[idx_state].F_5[idx_grid] ) );
	...
	for( idx_state = 0; idx_state < 2*N_states[1]; idx_state++ ) // Write out conduction bands in order of increasing energy
	{
		...
		strcpy( str1, "envelope_function_cb_" );
		switch( direction )
		{
			...
			case 4: // Linescan along [101]
				...
				size = sprintf( str2, "%d_101.dat", idx_state + 1 );

				break;

			case 5: // Linescan along [011]

				size = sprintf( str2, "%d_011.dat", idx_state + 1 );
		...
		for( idx_grid = 0; idx_grid < N_grid; idx_grid++ )
		{
			...
			fprintf( fp1, "%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\n",
														0.1*x[idx_grid], 0.1*y[idx_grid], 0.1*z[idx_grid], eigenstate_cb[idx_state].E,
														sqrt( 1000.0 )*creal( eigenstate_cb[idx_state].F_4[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_cb[idx_state].F_4[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_cb[idx_state].F_8[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_cb[idx_state].F_8[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_cb[idx_state].F_3[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_cb[idx_state].F_3[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_cb[idx_state].F_7[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_cb[idx_state].F_7[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_cb[idx_state].F_2[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_cb[idx_state].F_2[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_cb[idx_state].F_6[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_cb[idx_state].F_6[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_cb[idx_state].F_1[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_cb[idx_state].F_1[idx_grid] ),
														sqrt( 1000.0 )*creal( eigenstate_cb[idx_state].F_5[idx_grid] ),
														sqrt( 1000.0 )*cimag( eigenstate_cb[idx_state].F_5[idx_grid] ) );
