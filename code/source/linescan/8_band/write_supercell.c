...
void write_supercell( int N_shape, double T, double Fx, double Fy, double Fz, struct supercell_geometry *supercell, struct material_params *parameters )
{
	...
	switch( isotropic )
	{
		...
		case 0:
			...
			fprintf( fp1, "The full, anisotropic elastic tensor is employed in the strain field calculations" );

			break;

		case 1:

			fprintf( fp1, "The isotropic approximation to the elastic tensor is employed in the strain field calculations" );
	...
	switch( piezo_order )
	{
		...
		case 0:
			...
			fprintf( fp1, "Not computing piezoelectric potential\n" );

			break;

		case 1:

			fprintf( fp1, "Computing first order piezoelectric potential only\n" );

			break;

		case 2:

			fprintf( fp1, "Computing first and second order piezoelectric potential\n" );

			break;

		case 3:

			fprintf( fp1, "Computing second order piezoelectric potential only\n" );
	...
	if( supercell[0].shape == 0 )
	{
		...
		fprintf( fp1, "\n" );
		fprintf( fp1, "\tCentre (x, y, z)         [nm] = (%.1f, %.1f, %.1f)\n", 0.1*supercell[0].Ox, 0.1*supercell[0].Oy, 0.1*supercell[0].Oz );
		fprintf( fp1, "\tSupercell length along x [nm] = %.1f\n", 0.1*supercell[0].L1 );
		fprintf( fp1, "\tSupercell length along y [nm] = %.1f\n", 0.1*supercell[0].L2 );
