...
complex double characteristic_function_fourier( double Gx0, double Gy0, double Gz0, struct supercell_geometry *supercell, int idx_shape, int strain_region )
{

	int supercell_size = N_shape*sizeof( _supercell_geometry );
	...
	if ( strain_region == 0 || idx_shape == 0 )
	{
	...
	if( supercell[idx_shape].shape == 10 )
	{
		...
		result = characteristic_function_fourier( Gx0, Gy0, Gz0, supercell_copy, idx_shape, strain_region );
	...
	if( supercell[idx_shape].shape == 13 )
	{
		...
		result -= cexp( -0.5*I*Gz*supercell[idx_shape].L4 )*characteristic_function_fourier( Gx0, Gy0, Gz0, supercell_copy, idx_shape, strain_region );
