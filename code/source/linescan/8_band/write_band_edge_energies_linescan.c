...
void write_band_edge_energies_linescan( int direction, int N_grid, double *x, double *y, double *z, struct band_edge_energies band_edges )
{
	...
	// Write out linescan of the components of the band edge energies along the specified direction

	int idx_grid, size = 80;
	char str1[size], str2[size];
	FILE *fp1;

	strcpy( str1, "band_edge_energies_linescan_" );
	...
	for( idx_grid = 0; idx_grid < N_grid; idx_grid++ )
	{
		...
		fprintf( fp1, "%13e\t%13e\t%13e\t%13e\t%13e\t%13e\t%13e\n", 0.1*x[idx_grid], 0.1*y[idx_grid], 0.1*z[idx_grid], band_edges.E_so[idx_grid], band_edges.E_lh[idx_grid], band_edges.E_hh[idx_grid], band_edges.E_cb[idx_grid] );
